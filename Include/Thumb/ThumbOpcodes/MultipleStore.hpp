...
public:
    ...
    static constexpr uint16_t mask(const uint16_t opcode) { return 0; }
    ...
    static constexpr MultipleStoreOpcode fromOpcode(const uint16_t opcode)
    {
        return {
            ...
            .baseReg =
                static_cast<uint16_t>((opcode >> 8) & 0x0007), // bits 8–11
            .reserved =
                static_cast<uint16_t>((opcode >> 11) & 0x001F) // bits 12–15
        };
    }
    ...
    static void execute(Registers& regs, const uint16_t opcode)
    {
        ...
        const uint32_t bits =
            std::popcount(op.rlist & ((1ULL << op.baseReg) - 1ULL));
    ...
    static auto createPlainReg(uint32_t s) { return std::format("R{}", s); }
