...
public:
    ...
    static constexpr SpRelativeOpsOpcode fromOpcode(const uint16_t opcode)
    {
        return {
            ...
            .destSourceReg =
                static_cast<uint16_t>((opcode >> 8) & 0x0007), // bits 8–10
            .loadFlag =
    ...
    static void execute(Registers& regs, const uint16_t opcode)
    {
        ...
        if constexpr (c_op.loadFlag) {
            regs[op.destSourceReg] = loadFromAddress32(
                regs[TRegisters::EStackPointer] + (op.immediate << 2));
        } else {
            writeToAddress32(regs[TRegisters::EStackPointer] +
                                 (op.immediate << 2),
    ...
    static auto disassemble(const uint16_t opcode)
    {
        const auto op = fromOpcode(opcode);
        if (op.loadFlag) {
            return std::format("LDR R{}, [SP, #0x{:x}]", op.destSourceReg,
                               op.immediate << 2);
        }
        return std::format("STR R{}, [SP, #0x{:x}]", op.destSourceReg,
                           op.immediate << 2);
    }
};
