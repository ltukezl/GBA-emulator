...
public:
    ...
    static void execute(Registers& regs, const uint16_t opcode)
    {
        ...
        if constexpr (c_op.Sub) {
            regs[op.destination] =
                mathOps::Sub::calculate(regs.m_cpsr, reg_value, value);
            mathOps::Sub::calcConditions(regs.m_cpsr, regs[op.destination],
                                         reg_value, value);
        } else {
            regs[op.destination] =
                mathOps::Add::calculate(regs.m_cpsr, reg_value, value);
            mathOps::Add::calcConditions(regs.m_cpsr, regs[op.destination],
                                         reg_value, value);
        }
    }
    ...
    static auto disassemble(const uint32_t opcode)
    {
        ...
        return std::format("{} R{}, R{}, {}{:x}", operation, op.destination,
                           op.source, prefix, op.regOrImmediate);
    }
};
