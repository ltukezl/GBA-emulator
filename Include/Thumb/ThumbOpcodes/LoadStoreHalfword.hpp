...
public:
    ...
    static constexpr LoadStoreHalfwordOpcode fromOpcode(const uint16_t opcode)
    {
        return {
            ...
            .offset =
                static_cast<uint16_t>((opcode >> 6) & 0x001F), // bits 6–10
            .loadFlag =
    ...
    static void execute(Registers& regs, const uint16_t opcode)
    {
        ...
        if constexpr (c_op.loadFlag) {
            regs[op.destSourceReg] =
                loadFromAddress16(regs[op.baseReg] + immediate);
        } else {
            writeToAddress16(regs[op.baseReg] + immediate,
                             regs[op.destSourceReg]);
        }
    }
    ...
    static auto disassemble(const uint16_t opcode)
    {
        ...
        const uint32_t immediate = op.offset << 1;
        if (op.loadFlag) {
            return std::format("LRDH R{}, [R{}, #{:x}]", op.destSourceReg,
                               op.baseReg, immediate);
        }
        return std::format("STRH R{}, [R{}, #{:x}]", op.destSourceReg,
                           op.baseReg, immediate);
    }
};
