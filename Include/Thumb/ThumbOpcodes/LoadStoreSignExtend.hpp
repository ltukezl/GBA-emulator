...
public:
    ...
    static constexpr LoadStoreSignExtendOpcode fromOpcode(const uint16_t opcode)
    {
        return {
            ...
            .baseReg =
                static_cast<uint16_t>((opcode >> 3) & 0x0007), // bits 3–5
            .offsetReg =
                static_cast<uint16_t>((opcode >> 6) & 0x0007), // bits 6–8
            .reserved1 = static_cast<uint16_t>((opcode >> 9) & 0x0001), // bit 9
            .extend = static_cast<uint16_t>((opcode >> 10) & 0x0001), // bit 10
            .halfWord =
                static_cast<uint16_t>((opcode >> 11) & 0x0001), // bit 11
            .reserved2 =
                static_cast<uint16_t>((opcode >> 12) & 0x000F) // bits 12–15
        };
    }
    ...
    static constexpr bool isThisOpcode(const uint16_t opcode)
    {
        ...
        return (opcodeStruct.reserved1 == 1) &&
            (opcodeStruct.reserved2 == 0b0101);
    }
    ...
    static void execute(Registers& regs, const uint16_t opcode)
    {
        ...
        if constexpr (c_op.halfWord && !c_op.extend) {
            ...
            if (c_op.halfWord && !(totalAddress & 1)) {
                regs[op.destSourceReg] = static_cast<int32_t>(
                    static_cast<int16_t>(loadFromAddress16(totalAddress)));
            } else {
                regs[op.destSourceReg] = static_cast<int32_t>(
                    static_cast<int8_t>(loadFromAddress(totalAddress)));
            }
        }
    }
    ...
    static auto disassemble(const uint16_t opcode)
    {
        ...
        if (op.halfWord && op.extend) {
            return std::format("LDRSH R{}, [R{}, R{}]", op.destSourceReg,
                               op.baseReg, op.offsetReg);
        }
        if (op.halfWord && !op.extend) {
            return std::format("LDRH R{}, [R{}, R{}]", op.destSourceReg,
                               op.baseReg, op.offsetReg);
        }
        if (!op.halfWord && op.extend) {
            return std::format("LDRSB R{}, [R{}, R{}]", op.destSourceReg,
                               op.baseReg, op.offsetReg);
        }
        return std::format("STRH R{}, [R{}, R{}]", op.destSourceReg, op.baseReg,
                           op.offsetReg);
    }
};
