...
public:
    ...
    uint32_t read32_impl(const Registers& registers,
    ...
    std::array<uint8_t*, 4> m_fullRegion{
        m_memoryArea.data(), m_bgAndTileRegion2.data(), m_objectRegion.data(),
        m_objectRegion.data()};
    TilesetMemoryObserver m_observer;
private:
    template<typename T>
    constexpr T& as(const uint32_t addr, const uint32_t region)
    {
        return *reinterpret_cast<T*>(m_fullRegion[region] + addr);
    }
};
