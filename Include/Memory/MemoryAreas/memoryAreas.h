...
union MemoryAddress
{
    ...
    MemoryAddress(const uint32_t mask, const uint32_t value) :
        address{value}, mask{mask}
    {
    }
    ...
    const MemoryAddress operator%(const uint32_t value) const
    {
        return MemoryAddress{raw % value};
    }
    const MemoryAddress operator-(const uint32_t value) const
    {
        return MemoryAddress{raw - value};
    ...
    const MemoryAddress operator&(const uint32_t value) const
    {
        return MemoryAddress{mask, address & value};
    }
 ...
 *
 *     void consumeCycles(uint8_t memoryAreaCycles) { counter+=2;
 *counter+=memoryAreaCycles; };
 * }
 *
 */
...
public:
    template<typename... T>
    auto read8(const T... params)
    {
        return get()->read8_impl(params...);
    }

    template<typename... T>
    auto read16(const T... params)
    {
        return get()->read16_impl(params...);
    }

    template<typename... T>
    auto read32(const T... params)
    {
        return get()->read32_impl(params...);
    }
    ...
    template<typename T>
    constexpr T& as(const uint32_t addr)
    {
        return *reinterpret_cast<T*>(get()->m_memoryArea.data() + addr);
    }
    ...
    constexpr auto& operator[](const uint32_t offset)
    {
        return getMemoryPtr()[offset];
    }

private:
    constexpr Derived* get() { return static_cast<Derived*>(this); }
};
