...
public:
    ...
    static constexpr MsrReg_t fromOpcode(const uint32_t opcode)
    {
        return {
            .source_operand = static_cast<uint32_t>((opcode >> 0x00U) & 0x0FU),
            .reserved4 = static_cast<uint32_t>((opcode >> 0x04U) & 0xFFU),
    ...
    static constexpr bool isThisOpcode(const uint32_t opcode)
    {
        ...
        return (opcodeStruct.reserved1 == 0b1111) &&
            ...
            (opcodeStruct.reserved4 == 0);
    }

    static void execute(Registers& regs, const uint32_t opcode)
    {
        const auto op = fromOpcode(opcode);
        auto operand = regs[op.source_operand];
        if (op.source_operand == 15) {
            operand += 4;
        }
        ...
        if (op.destination) {
            ...
            if (regs.m_cpsr.mode != CpuModes_t::EUSR &&
                ...
                if (op.ctl) {
                    ...
                    tmp_cpsr1.thumb = tmp_cpsr.thumb;
            ...
            if (regs.m_cpsr.mode != CpuModes_t::EUSR && op.ctl) {
                ...
                regs.m_cpsr.thumb = tmp_cpsr.thumb;
                regs.m_cpsr.mode = static_cast<CpuModes_t>(
                    (static_cast<uint8_t>(regs.m_cpsr.mode)) | 0x10);
