...
public:
    ...
    {
        return {.offset = static_cast<int32_t>(opcode & 0xFFFFFF),
    ...
    static void execute(Registers& regs, const uint32_t opcode)
    {
        ...
        if constexpr (c_op.link == BL_t::EBranchAndLink) {
            regs[TRegisters::ELinkRegisterLR] =
                regs[TRegisters::EProgramCounter];
        }
        regs[TRegisters::EProgramCounter] += 4;
        const uint32_t location =
            (static_cast<uint32_t>(static_cast<int32_t>(op.offset)) << 2);
    ...
    static auto disassemble(const uint32_t opcode)
    {
        ...
        return std::format("{}{} #0x{:x}", ls, condition_strings[op.condition],
                           location);
    }
};
