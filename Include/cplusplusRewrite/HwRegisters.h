...
public:
    ...
    const std::array<uint32_t*, REG_LEN>* r;
    ...
    const std::array<uint32_t*, REG_LEN> usrSys = {
        &sharedRegs[0],   &sharedRegs[1],   &sharedRegs[2],   &sharedRegs[3],
        &sharedRegs[4],   &sharedRegs[5],   &sharedRegs[6],   &sharedRegs[7],
        &extRegisters[0], &extRegisters[1], &extRegisters[2], &extRegisters[3],
        &extRegisters[4], &usrBanked[0],    &usrBanked[1],    &sharedRegs[8],
        &m_cpsr.val};

    const std::array<uint32_t*, REG_LEN> svc = {
        &sharedRegs[0],   &sharedRegs[1],   &sharedRegs[2],   &sharedRegs[3],
        &sharedRegs[4],   &sharedRegs[5],   &sharedRegs[6],   &sharedRegs[7],
        &extRegisters[0], &extRegisters[1], &extRegisters[2], &extRegisters[3],
        &extRegisters[4], &svcBanked[0],    &svcBanked[1],    &sharedRegs[8],
        &sprs_svc};

    const std::array<uint32_t*, REG_LEN> abt = {
        &sharedRegs[0],   &sharedRegs[1],   &sharedRegs[2],   &sharedRegs[3],
        &sharedRegs[4],   &sharedRegs[5],   &sharedRegs[6],   &sharedRegs[7],
        &extRegisters[0], &extRegisters[1], &extRegisters[2], &extRegisters[3],
        &extRegisters[4], &abtBanked[0],    &abtBanked[1],    &sharedRegs[8],
        &sprs_abt};

    const std::array<uint32_t*, REG_LEN> fiq = {
        &sharedRegs[0], &sharedRegs[1], &sharedRegs[2], &sharedRegs[3],
        &sharedRegs[4], &sharedRegs[5], &sharedRegs[6], &sharedRegs[7],
        &fiqBanked[0],  &fiqBanked[1],  &fiqBanked[2],  &fiqBanked[3],
        &fiqBanked[4],  &fiqBanked[5],  &fiqBanked[6],  &sharedRegs[8],
        &sprs_fiq};

    const std::array<uint32_t*, REG_LEN> irq = {
        &sharedRegs[0],   &sharedRegs[1],   &sharedRegs[2],   &sharedRegs[3],
        &sharedRegs[4],   &sharedRegs[5],   &sharedRegs[6],   &sharedRegs[7],
        &extRegisters[0], &extRegisters[1], &extRegisters[2], &extRegisters[3],
        &extRegisters[4], &irqBanked[0],    &irqBanked[1],    &sharedRegs[8],
        &sprs_irq};

    const std::array<uint32_t*, REG_LEN> undef = {
        &sharedRegs[0],   &sharedRegs[1],   &sharedRegs[2],   &sharedRegs[3],
        &sharedRegs[4],   &sharedRegs[5],   &sharedRegs[6],   &sharedRegs[7],
        &extRegisters[0], &extRegisters[1], &extRegisters[2], &extRegisters[3],
        &extRegisters[4], &undBanked[0],    &undBanked[1],    &sharedRegs[8],
        &sprs_udf};
    ...
    Registers(const CpuModes_t mode = CpuModes_t::ESUPER) { reset(mode); }

    Registers(const std::initializer_list<uint32_t> list,
              const CpuModes_t mode = CpuModes_t::ESUPER)
    {
        reset(mode);
        std::copy(list.begin(), list.end(), *r->begin());
    }

    uint32_t& operator[](const uint32_t index) const { return *(*r)[index]; }
    ...
    void nextProgramCounter() { *(*r)[EProgramCounter] += m_offset; }

    void prevProgramCounter() { *(*r)[EProgramCounter] -= m_offset; }
};
