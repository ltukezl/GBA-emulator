...
void updateTimers(uint64_t cycles)
{
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (timerCtrl->startStop) {
            overflowed = ((timerCtrl->counterVal + cycles) & 0xFFFF) <=
                timerCtrl->counterVal;
            ...
            if (overflowed) {
                ...
                if (i < 4 && timerCtrl->timing && timerCtrl->startStop) {
                    // uint16_t oldVal = loadFromAddress16(0x4000100 + 4 * (i +
                    // 1));
                    // reinterpret_cast<uint16_t**>(memoryLayout)[4][0x100 + 4 *
                    // (i + 1)] = oldVal + 1;
                }
