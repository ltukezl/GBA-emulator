...
decltype(&ImmediateRotater::calculate)
BarrelShifterDecoder::decode(const uint32_t opCode)
{
...
decltype(&ImmediateRotater::disassemble)
BarrelShifterDecoder::disassemble(const uint32_t opCode)
{
    const auto decoder = std::bit_cast<DecodeBits>(opCode);
    if (decoder.opCode == 0) {
        if (decoder.immediate) {
