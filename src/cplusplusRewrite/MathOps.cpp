...
static bool subCarry(const uint32_t operand1,
                     const uint32_t operand2,
                     const uint32_t carryf)
{
    const uint64_t adjust =
        static_cast<uint64_t>(operand1) + static_cast<uint64_t>(carryf);
    return static_cast<uint64_t>(operand2) >= adjust;
}

static bool subOverflow(const uint32_t operand1,
                        const uint32_t operand2,
                        const uint32_t result)
{
    return static_cast<int32_t>((operand1 ^ operand2) & (operand2 ^ result)) <
        0;
}
