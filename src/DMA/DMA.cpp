...
void doDMA(DMAcontrol* DMAControl,
           uint32_t i,
           ...
           uint32_t wordCount)
{
    if ((DMAControl->transferType == DMAtransfer_t::WORD) &&
        (DMAControl->destCtrl == DMAcontrol_t::Increment ||
         DMAControl->destCtrl == DMAcontrol_t::IncOrReload) &&
            ...
            if (DMAControl->transferType == DMAtransfer_t::HALFWORD) {
                if (sourceAddress.mask > EGamePak6 && i == 0) {
                    ...
                    writeToAddress16(
                        ...
                        loadFromAddress16(sourceAddress.raw & (~1)));
                }
            } else {
                if (sourceAddress.mask > EGamePak6 && i == 0) {
                    ...
                    writeToAddress32(
                        ...
                        loadFromAddress32(sourceAddress.raw & (~3)));
                }
            }

            if (DMAControl->destCtrl == DMAcontrol_t::Increment ||
                DMAControl->destCtrl == DMAcontrol_t::IncOrReload) {
                destinationAddress = destinationAddress +
                    (DMAControl->transferType == DMAtransfer_t::WORD ? 4 : 2);
...
void startDMA()
{
    for (int i = 0; i < 4; i++) {
        ...
        if ((DMAControl->enable) ||
            (InterruptFlagRegister->hBlank && DMAControl->irq == 2) ||
            (InterruptFlagRegister->vBlank && DMAControl->irq == 1)) {
            if (DMAControl->timing == DMAtiming_t::VBlank &&
                !InterruptFlagRegister->vBlank) {
                continue;
            }
            if (DMAControl->timing == DMAtiming_t::VBlank &&
                !InterruptFlagRegister->vBlank) {
            ...
            if ((i == 1 || i == 2) &&
                DMAControl->timing == DMAtiming_t::Special) {
                for (int words = 0; words < 4; words++) {
                    // writeToAddress32(destinationAddress + 4,
                    // loadFromAddress32(sourceAddress)); //fifo mode, do addr
                    // control logic sound control. do later
                }
                continue;
            }
            ...
            if (destinationAddress.mask >= EGamePak1 &&
                destinationAddress.mask <= EGamePak6) {
                continue;
            }

            if (i != 3) {
                if (sourceAddress.mask >= EGamePak1 &&
                    sourceAddress.mask <= EGamePak6) {
                    continue;
                }
