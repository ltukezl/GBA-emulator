...
constexpr std::array<uint32_t, 16> memsizes = {
    0x4000,    0x4000,    0x40000,   0x8000,    0x400,     0x400,
    0x20000,   0x400,     0x1000000, 0x1000000, 0x1000000, 0x1000000,
    0x1000000, 0x1000000, 0x400000,  0x400000};
...
std::array<unsigned char*, 16> memoryLayout = {[]() constexpr {
    std::array<unsigned char*, 16> retArray{
        nullptr, nullptr, nullptr, nullptr, IoRAM,   nullptr, nullptr, nullptr,
...
bool isInterrupt()
{
    return r.m_cpsr.mode == CpuModes_t::ESUPER ||
...
bool specialWrites(uint32_t mask, uint32_t addr, uint32_t val)
{
    if (mask == 4 && addr == 0x202) { // interrupt flag clear
        ...
        return true;
               ...
               (addr == 0x100 || addr == 0x104 || addr == 0x108 ||
                addr == 0x10B)) {
        return timerReloadWrite(addr, val);
    } else if (mask == 4 &&
               (addr == 0x102 || addr == 0x106 || addr == 0x10A ||
                addr == 0x10D)) {
                   ...
                   uint32_t size)
{
    ...
    uint32_t destination_clamped =
        clampAddress(destination.mask, destination.alignedMasked32b());
    uint32_t source_clamped =
        clampAddress(source.mask, source.alignedMasked32b());

    if (source.mask > EGamePak6 && dmaNumber != 0) {
        uint8_t sramVal = sram.read8(source.alignedMasked32b());
        memset(&memoryLayout[destination.mask][destination_clamped], sramVal,
               size * 4);
    } else if (source.mask > EGamePak6 && dmaNumber == 0) {
        memset(&memoryLayout[destination.mask][destination_clamped], 0,
               size * 4);
    } else {
        memcpy(&memoryLayout[destination.mask][destination_clamped],
               &memoryLayout[source.mask][source_clamped], size * 4);
    }
}
...
void writeToAddress(uint32_t address, uint8_t value)
{
    ...
    if (memDecoder.mask == 4 &&
        memDecoder.address > memsizes[memDecoder.mask]) {
        return;
    }

    else if ((memDecoder.mask == 5 || memDecoder.mask == 6 ||
              memDecoder.mask == 7) &&
             (displayCtrl->forceBlank || InterruptFlagRegister->vBlank ||
              InterruptFlagRegister->hBlank)) {
        return;
    }
    ...
    if (memDecoder.mask == 7 ||
        (displayCtrl->bgMode == 7 && memDecoder.mask == 6 &&
         address >= 0x10000) ||
        (displayCtrl->bgMode == 4 && memDecoder.mask == 6 &&
         address >= 0x10000)) {
        return;
    }

    if ((displayCtrl->bgMode == 4 && memDecoder.mask == 6 &&
         address < 0x10000) ||
        memDecoder.mask == 5) {
...
uint32_t loadFromAddress32(uint32_t address, bool free)
{
    ...
    if (r[15] < 0x4000) {
        ...
        result =
            RORnoCond(rawLoad32(memoryLayout[mask], address - misalignment),
                      8 * misalignment);
    }
