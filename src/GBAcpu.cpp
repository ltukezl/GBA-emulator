...
static void readFile(const std::string& fileName,
                     std::array<uint8_t, 0x4000>& input)
{
    size_t size = std::filesystem::file_size(fileName);
...
int main(int argc, char* args[])
{
    ...
    rawWrite16(IoRAM, 0x130,
               0xFFFF); // input register, 0 = pressed down, 1 = released
...
#else
    memoryLayout[4][6] =
        0x9A; // Vcount initializtion use 7e when cycle counting is ready
#endif
    memoryInits();
    ...
    while (true) {
        ...
        uint32_t opCode = r.m_cpsr.thumb
            ? loadFromAddress16(ProgramCounter, true)
            : loadFromAddress32(ProgramCounter, true);
